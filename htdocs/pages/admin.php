...
<section style="margin-left:300px">
    ...
    <form action="../process/admin/add_tourOperator.php" method="post">
            <div class="mb-3">
                <label for="name" class="form-label">Add destination</label>
                <input type="text" class="form-control" name="name" id="name" aria-describedby="nameTip">
                <div id="nameTip" class="form-text">Please enter the name of the new Destination.</div>
            </div>
            <!-- input type texte pour le prix du voyage  -->
            <input type="text" name="price" placeholder="addPrice $">

           
            <!-- select avec les otpion dans un foreach pour les tour Opearteur -->

            <select name="TourOperator" id="TourOperateur-select">

                        <?php foreach ($TourOperator as $key => $value) {
                                ?>
                                <option value=""></option>    
                        <?php } ?>                
            </select> 
